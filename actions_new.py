...
class ActionGetComplaintDetail(FormAction):
    ...
    def submit(self, dispatcher, tracker, domain):
        ...
        if time:
            day_dict = {'monday' : 0, 'tuseday' : 1, 'wednesday' : 2, 'thursday' : 3 , 'friday' :  4, 'saturday' : 5, 'sunday' : 6}
            ...
            data[pin_code][technician_dict[technician]]['day'][day_dict[day]] = True

            with open('data.txt', 'w') as outfile:
                json.dump(data, outfile)

        return [SlotSet("trackid", trackidfinal), SlotSet("time", None)]
...
class SetGeoLocationAddress(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        confirm = tracker.get_slot('confirm_geo_location')
        if confirm:
            return []
...
def generate_timeslots(tracker, pin_code):
    ...
    if count == 3:
        for i in range(3):
            ...
            time_slots.append(technician_dict[technician_choice[i]] + "\t" +  day_dict[day] + "\t" + str(time) + "-" + str(time + (time_taken if time_taken is not None else str(time + 2))) + (' am' if am_pm == 1 else ' pm'))
...
class SetTimeSlot(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if time is not None:
            ...
            return [SlotSet("date", day), SlotSet("time_slots", time_slot), SlotSet("time1", None), SlotSet("time2", None), SlotSet("time3", None)]

        return [SlotSet("time1", None), SlotSet("time2", None), SlotSet("time3", None), SlotSet("time", None)]
